...
/**
 * @class WeatherData weather_data.h "data/weather_data.h"
 * @brief This class represents a weather data reading, where measurements may or may not be present.
 */
class WeatherData {
public:

    /** @brief Declare the type for a Unix seconds timestamp */
    using data_time = std::chrono::seconds::rep;

    /** @brief The data's (GMT/UTC) Unix timestamp in seconds, if available */
